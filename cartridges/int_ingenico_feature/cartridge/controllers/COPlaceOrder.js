...
function handlePayments(order) {
    // dw.system.Logger.getLogger("Ingenico").warn("GOT HERE OVERLOADED CONTROLLER: handlePayments");
    if (order.getTotalNetPrice() !== 0.00) {
        ...
        for (var i = 0; i < paymentInstruments.length; i++) {
            ...
            if (PaymentMgr.getPaymentMethod(paymentInstrument.getPaymentMethod()).getPaymentProcessor() === null) {
                ...
                // Added to handle 3D Secure and other user verification
                }else if(authorizationResult.end){
...
function start() {
    ...
    // Added to handle 3D Secure and other user verification
        ...
        session.getPrivacy().pendingOrderNo = order.getOrderNo();
        session.getPrivacy().pendingOrder = order;
        return {};
    }
